...
NTSTATUS DriverEntry(PDRIVER_OBJECT DriverObjct, PUNICODE_STRING RegistryPath)
{
	...
	char hook1[] = { 0xE9,0xB9,0x02,0x00,0x00,0x90 };
	char hook2[] = { 0xE9,0xF8,0x03,0x00,0x00,0x90 };
	char hook3[] = { 0xE9,0x2D,0x01,0x00,0x00,0x90 };
	/*char hook4[] = { 0xE9,0x96,0x00,0x00,0x00,0x90 };
	char hook5[] = { 0xE9,0xA8,0x00,0x00,0x00,0x90 };
	char hook6[] = { 176,1 };
	char hook7[] = { 0x90,0x90 };*/
	RtlInitUnicodeString(&hookdllname, L"\\FileSystem\\MessageTransfer");
	...
	if (NT_SUCCESS(status))
	{
		...
		//	hook csrss.exe的判断
		/*pTemp = (ULONG64)mStart + 0x396C39;
		Mdl_RWMemory((PVOID)pTemp, rProcess, 12, TRUE);*/
		//	hook 创建进程回调
		pTemp = (ULONG64)mStart + 0x39675E;
		Mdl_RWMemory((PVOID)pTemp, hook1, 1, TRUE);
		//	hook 加载模块回调
		pTemp = (ULONG64)mStart + 0x39639F;
